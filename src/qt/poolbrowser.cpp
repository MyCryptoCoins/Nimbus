...
const QString apiBittrexTrades = "http://bittrex.com/api/v1.1/public/getmarkethistory?market=BTC-NMB&count=100";
const QString apiBittrexOrders = "http://bittrex.com/api/v1.1/public/getorderbook?market=BTC-NMB&type=both&depth=50";
...
void PoolBrowser::bittrexMarketSummary(QNetworkReply* response)
{
    ...
    for(int i = 0; i < qslApiResponse.count(); i++){
        ...
        if(read_string(qslApiResponse[i].toStdString(), jsonResponse)) {
            ...
            if (getPairValue(jsonObject, "MarketName").get_str() == "BTC-NMB") {
                try {
